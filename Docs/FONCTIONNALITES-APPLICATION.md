# Fonctionnalités d'Amaliassistant - Documentation de référence

**Dernière mise à jour** : Version 1.9.0

Ce document décrit précisément les fonctionnalités de l'application Amaliassistant pour éviter les répétitions et assurer la cohérence dans les futures versions.
...
1. **Classes d'invocations** : Utiliser "Zobal" (et non "Masqueraider"), Sadida, Osamodas, etc.
2. **Loot Tracker** : Mentionner que l'interface est appelée à évoluer
3. **Notifications de ventes** : Positionnables via clic droit sur le menu de l'application
4. **Volume sonore** : Réglable dans les paramètres (pas seulement activable/désactivable)
5. **Tracking XP** : Ne pas mentionner dans les fonctionnalités principales (fonctionnalité secondaire ou interne)
6. **Windows** : "Adapté pour Windows 11, tout en restant parfaitement fonctionnel sous Windows 10"
